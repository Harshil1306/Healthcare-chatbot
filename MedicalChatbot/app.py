...
doctors_dic = doctors_desc.to_dict()
...
def predict_symptom():
    ...
    if sentence.replace(".", "").replace("!","").lower().strip() == "done":
        ...
        if not user_symptoms:
            ...
            for x,y in doctors_dic["Disease"].items():
                for x,y in doctors_dic["Disease"].items():
                    if y==disease:
                        ans = doctors_dic["Doctor"][x]
                        ans2 = f'<a href="{doctors_dic["Link"][x]}">{doctors_dic["Link"][x]}</a>'
            print(ans)
            ...
            response_sentence = "It looks to me like you have " + disease + ". <br><br> <i>Description: " + description + "</i>" + "<br><br><b>"+ precautions + "</b>" + "<br><br><b>" + ans + "</b>"+ " " +"specializes in treating"+" "+ "<b>" + disease+ "</b>" +" "+"Follow the link to contact him/her"+"<br><br>"+ ans2
            ...
            if np.mean(severity) > 4 or np.max(severity) > 5:
                response_sentence = response_sentence + "<br><br>Considering your symptoms are severe, and Cindy isn't a real doctor, you should consider talking to one. :)"
